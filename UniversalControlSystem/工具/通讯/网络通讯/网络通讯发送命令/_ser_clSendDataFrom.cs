...
{
    public partial class _ser_clSendDataFrom : Form
    {
        public _ser_clSendDataFrom()
        {
            InitializeComponent();
        }
        ...
        int Num = 0;
        _ser_clSendData _ser_clSendData /*= new _OutPut_Puse()*/;
        public void ShowMesage(string Name,string StepName, int num)
        {
            ...
            _ser_clSendData = (_ser_clSendData)tools[Num];
            _通讯名字.Text = _ser_clSendData._通讯名字;
            通讯选择.Text = _ser_clSendData.通讯选择;
            _ResData.Text = _ser_clSendData._SendData;
            if (_ser_clSendData.FlowChar_StepControlLoop == Num)
            {
                _ser_clSendData.FlowChar_StepControlLoop = Num;
                步骤跳转.Text = Num.ToString();
            }
            else
            {
                步骤跳转.Text = _ser_clSendData.FlowChar_StepControlLoop.ToString();
            }
            try
            {                
            }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                _ser_clSendData.SendData(_ser_clSendData);
            }
            catch (Exception ex)
            {
            }
        ...
        private void _通讯名字_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                tools = LoadData._ImageToolRun[TOOLName.Text];
                _ser_clSendData = (_ser_clSendData)tools[Step];
                _ser_clSendData._通讯名字 = _通讯名字.Text;
            }
            catch (Exception)
            {
            }
        ...
        private void 通讯选择_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                tools = LoadData._ImageToolRun[TOOLName.Text];
                _ser_clSendData = (_ser_clSendData)tools[Step];
                _ser_clSendData.通讯选择 = 通讯选择.Text;
            }
            catch (Exception)
            {
            }
        ...
        private void _ResData_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                tools = LoadData._ImageToolRun[TOOLName.Text];
                _ser_clSendData = (_ser_clSendData)tools[Step];
                _ser_clSendData._SendData = _ResData.Text;
            }
            catch (Exception)
            {
            }
        ...
        private void 步骤跳转_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                _ser_clSendData = (_ser_clSendData)tools[Step];
                _ser_clSendData.FlowChar_StepControlLoop = int.Parse(步骤跳转.Text);
            }
            catch (Exception)
            {
            }
