...
{  
    ...
    public partial class ImgFtpUpd
    {
        ...
        public string UploadFile(string ftpPath, string path, string StrFileName, string SN)
        {
            float percent = 0;
            ...
            bool c = MakeDir(ftpPath + SN);
            if (c == false)
            {
                return "false";
            }
            path = FTPConstr + ftpPath + SN + "/" + StrFileName;
        ...
        public string Upload(string ImgPath, string SN)
        {
            string StrFileName = Path.GetFileName(ImgPath);//图片路径
            //按日期命名文件夹
            string StrftpPath = DateTime.Now.ToString("yyyyMMdd");
            string path = UploadFile(StrftpPath + "/", ImgPath, StrFileName, SN);
            return path;
        }
